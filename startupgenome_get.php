...
$interval_query = mysqli_query($GLOBALS['cxn'], "SELECT sg_lastupdate FROM settings LIMIT 1");
  ...
  if((time()-$interval_info[sg_lastupdate]) > $sg_frequency || $_GET['override'] == "true") {
    ...
    try {
      ...
      foreach ($places_arr['response'] as $key => $place) {
        ...
        $place_query = mysqli_query($GLOBALS['cxn'], "SELECT id FROM places WHERE sg_organization_id='".$place['organization_id']."' LIMIT 1") or die(mysqli_error($GLOBALS['cxn']));

        // organization doesn't exist, add it to the db
        if(mysqli_num_rows($place_query) == 0) {
          mysqli_query($GLOBALS['cxn'], "INSERT INTO places (approved,
                                          title,
                                          type,
                                          lat,
                                          lng,
                                          ...
                                          )") or die(mysqli_error($GLOBALS['cxn']));

        // organization already exists, update it with new info if necessary
        } else if(mysqli_num_rows($place_query) == 1) {
          ...
          if($place_info['title'] != $place['name'] || $place_info['type'] != $place['type'] || $place_info['lat'] != $place['latitude'] || $place_info['lng'] != $place['longitude'] || $place_info['address'] != $place['address'] || $place_info['uri'] != $place['url'] || $place_info['description'] != $place['description']) {
            mysqli_query($GLOBALS['cxn'], "UPDATE places SET title='".parseInput($place['name'])."',
      ...
      $deleted = mysqli_query($GLOBALS['cxn'], "DELETE FROM places WHERE sg_organization_id NOT IN ({$org_array})") or die(mysqli_error($GLOBALS['cxn']));

      // update settings table with the timestamp for this sync
      mysqli_query($GLOBALS['cxn'], "UPDATE settings SET sg_lastupdate='".time()."'");
