      ...
      function initialize() {
        ...
        <?php
          ...
          foreach($types as $type) {
            $places = mysqli_query($GLOBALS['cxn'], "SELECT * FROM places WHERE approved='1' AND type='$type[0]' ORDER BY title");
          ...
          if($show_events == true) {
            $place[type] = "event";
            $events = mysqli_query($GLOBALS['cxn'], "SELECT * FROM events WHERE start_date > ".time()." AND start_date < ".(time()+9676800)." ORDER BY id DESC");
    ...
    <div class="menu" id="menu">
      <ul class="list" id="list">
        <?php
          ...
          foreach($types as $type) {
            if($type[0] != "event") {
              $markers = mysqli_query($GLOBALS['cxn'], "SELECT * FROM places WHERE approved='1' AND type='$type[0]' ORDER BY title");
            } else {
              $markers = mysqli_query($GLOBALS['cxn'], "SELECT * FROM events WHERE start_date > ".time()." AND start_date < ".(time()+4838400)." ORDER BY id DESC");
            }
    ...
    <div class="modal hide" id="modal_info">
      ...
      <div class="modal-body">
        <p>
          ...
          in our beloved Eindhoven. We've seeded the map but we need
          ...
          <?php } ?>
          Let's put Eindhoven on the map together!
        </p>
        <p>
        Questions? Feedback? Connect with us: <a href="http://www.twitter.com/<?= $twitter['username'] ?>" target="_blank">@<?= $twitter['username'] ?></a>
        </p>
        <p>
          Visit the <a href="http://github.com/eindhovenstartups/map">public GitHub repository</a> to contribute to this project.
        </p>
      </div>
