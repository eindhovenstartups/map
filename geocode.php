...
function geocode($table) {
  ...
  $result = mysqli_query($GLOBALS['cxn'], "SELECT * FROM $table WHERE lat=0 OR lng=0") or die(mysqli_error($GLOBALS['cxn']));
    ...
    while ($geocode_pending) {
      ...
      if ($status == "OK") {
        ...
        $query = sprintf("UPDATE $table " .
              " SET lat = '%s', lng = '%s' " .
              " WHERE id = '%s' LIMIT 1;",
              mysqli_real_escape_string($GLOBALS['cxn'], $lat),
              mysqli_real_escape_string($GLOBALS['cxn'], $lng),
              mysqli_real_escape_string($GLOBALS['cxn'], $id));
        $update_result = mysqli_query($GLOBALS['cxn'], $query);
        if (!$update_result) {
          die("Invalid query:");
        }
